...
export function StartFlow(){
  ...
  {step===1&&<><h1 className="display">What would you most like to change?</h1><p className="lead body-lg">This is not a test. One or two honest sentences are enough to give Michael a useful starting point.</p><div className="field" style={{marginTop:45}}><label htmlFor="goal" style={{color:"#8da2b7"}}>Your answer</label><textarea id="goal" value={goal} onChange={e=>setGoal(e.target.value)} style={{color:"white",borderColor:"rgba(255,255,255,.22)"}} placeholder="I want to understand…"/></div></>}
  {step===2&&<><h1 className="display">You&apos;ve given the conversation a starting point.</h1><p className="lead body-lg">The next step is to choose a consultation time or send Michael a direct message with the context you just clarified.</p><div style={{display:"flex",gap:12,flexWrap:"wrap",marginTop:40}}><Link className="button" href={`/book?utm_source=growthgains&utm_medium=website&utm_campaign=consultation&utm_content=start-flow&transition=${encodeURIComponent(choice)}&goal=${encodeURIComponent(goal)}`}>Choose a consultation time →</Link><Link className="button secondary" href={`/contact?transition=${encodeURIComponent(choice)}&goal=${encodeURIComponent(goal)}`}>Contact Michael directly ↗</Link></div></>}
